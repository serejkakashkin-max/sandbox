...
Sandbox - самостоятельное Flask/WSGI-приложение для публикации экспериментальных инструментов. Сейчас в него включены главная страница "Экспериментальные инструменты", веб-инструмент TA "Аудитор инцидентов", GD "Монитор релизов" и CA "Помощник ЗПИ".
...
- `CA - Частухин Александр/` - автономный Flask-проект помощника ЗПИ для сценарного разбора пользовательских логов.
...
Также поддерживается заголовок `X-Forwarded-Prefix`. Все URL, которые генерирует Flask, учитывают `SCRIPT_NAME`, поэтому приложение может работать за прокси, снимающим внешний префикс. CA подключен как отдельное WSGI-приложение и использует отдельное имя session-cookie `zpi_session`, чтобы не конфликтовать с соседними Flask-модулями.
...
После запуска приложение будет доступно на `http://127.0.0.1:3535/`, а CA - на `http://127.0.0.1:3535/ca/zpi-assistant/`.
...
TA сохраняет загруженные Excel-файлы в `cache/ta_incident_auditor/uploads/`. GD сохраняет primary snapshot, last-good snapshot и диагностический журнал в `GD/cache/`. Содержимое обоих runtime-каталогов не предназначено для Git. CA загруженные TXT/XLSX на диск не сохраняет: они разбираются в памяти текущего запроса.
...
CA следует этой схеме: исходный проект остается автономным внутри `CA - Частухин Александр/`, а корневой `wsgi.py` динамически загружает его Python-пакет и монтирует на `/ca/zpi-assistant`.
