...
Sandbox - самостоятельное Flask/WSGI-приложение для публикации экспериментальных инструментов. Сейчас в него включены главная страница "Экспериментальные инструменты", веб-инструмент TA "Аудитор инцидентов", GD "Монитор релизов", CA "Помощник ЗПИ" и MM "Генератор МПР".
...
- `CA/` - автономный Flask-проект помощника ЗПИ для сценарного разбора пользовательских логов.
- `MM/` - автономный генератор МПР, перенесенный из основного приложения Oplot; владелец `MM - Мухиддинов Ману`.
...
- `GET, POST /ca/zpi-assistant/` - помощник ЗПИ: загрузка TXT/XLSX и сценарный анализ логов.
- `GET /ca/zpi-assistant/health` - состояние приложения CA.
- `GET /mm/mpr/` - рабочая страница генератора МПР MM.
- `POST /mm/mpr/preview` - проверка XLSX/CSV и предпросмотр доступных комплектов.
- `POST /mm/mpr/generate` - генерация DOCX или ZIP МПР.
- `GET /mm/mpr/health` - состояние модуля MM.
...
- `ZPI_HTTPS` - включает Secure-флаг cookie CA; в production-конфигурации установлен в `1`.
- `ZPI_SECRET_KEY` - постоянный секрет Flask-сессии CA. В репозиторий не добавляется; на постоянном сервере рекомендуется задать через секреты окружения.
- `ZPI_MAX_UPLOAD_BYTES`, `ZPI_MAX_FILE_BYTES`, `ZPI_MAX_FILES` - необязательные лимиты загрузок CA; безопасные значения по умолчанию определены внутри приложения.

Также поддерживается заголовок `X-Forwarded-Prefix`. Все URL, которые генерирует Flask, учитывают `SCRIPT_NAME`, поэтому приложение может работать за прокси, снимающим внешний префикс. CA и MM подключены как отдельные WSGI-приложения; MM монтируется на `/mm/mpr`.
...
После запуска приложение будет доступно на `http://127.0.0.1:3535/`, CA - на `http://127.0.0.1:3535/ca/zpi-assistant/`, а MM - на `http://127.0.0.1:3535/mm/mpr/`.
...
CA также можно запускать автономно из его каталога:
...
В автономном режиме CA по умолчанию доступен на `http://127.0.0.1:5055/`.
...
TA сохраняет загруженные Excel-файлы в `cache/ta_incident_auditor/uploads/`. GD сохраняет primary snapshot, last-good snapshot и диагностический журнал в `GD/cache/`. Содержимое обоих runtime-каталогов не предназначено для Git. CA и MM обрабатывают пользовательские загрузки в памяти текущего запроса; MM сохраняет только сформированный результат в памяти до отправки пользователю.
...
CA и MM следуют этой схеме: исходные приложения остаются автономными внутри своих каталогов, а корневой `wsgi.py` монтирует их на собственные URL-префиксы.
